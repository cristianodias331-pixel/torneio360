...
-- O diretório inicial não deve transportar fotos Base64 de todas as arenas.
-- Fotos externas continuam leves; imagens incorporadas são buscadas somente
-- quando o cartão correspondente aparece na tela.
...
create or replace function public.get_public_arena_photo(p_organizer_id uuid)
returns text
language sql
stable
security definer
...
revoke all on function public.get_public_arena_photo(uuid) from public, anon, authenticated;
grant execute on function public.get_public_arena_photo(uuid) to anon, authenticated;

-- Capas Base64 permanecem salvas no torneio, mas deixam de fazer parte do
-- pacote inicial. Uma capa por URL continua no resumo por ser leve.
...
-- O perfil inicial contém apenas os cartões. O ranking completo de cada
-- circuito passa a ser obtido somente quando o visitante abre esse circuito.
      ...
      from (
        ...
        from (
          ...
          where history.circuit_id = circuit.id
            ...
            and exists (
              select 1
              from jsonb_array_elements_text(coalesce(to_jsonb(circuit.tournament_ids), '[]'::jsonb)) selected(value)
              join public.tournaments linked_tournament on linked_tournament.id::text = selected.value
              where selected.value = history.tournament_id::text
                and linked_tournament.user_id = circuit.user_id
                and coalesce(linked_tournament.data ->> 'deletedAt', '') = ''
            )
