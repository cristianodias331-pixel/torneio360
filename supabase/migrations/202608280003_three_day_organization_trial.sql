...
-- Novas ativações da identidade de organização recebem três dias corridos
-- de teste (hoje + dois dias). Ativações já iniciadas não são encurtadas.
...
declare
  ...
  trial_end date := (statement_timestamp() at time zone 'America/Sao_Paulo')::date + 2;
...
comment on function public.activate_my_organization() is
  'Ativa a organização na conta do atleta com três dias de teste, uma única vez.';
