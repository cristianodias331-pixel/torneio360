...
const modalityPickerDescriptions = {
...
function ModalityPicker({ value, onChange, options = [], disabled = false, legacyLabel = "" }) {
...
function buildPublicCircuitRankingGroups(circuit, tournaments = []) {
  ...
  tournaments
    ...
    .forEach((tournament) => {
      if (sourceCircuitIds.length > 0 && Array.isArray(circuit?.ranking_groups) && circuit.ranking_groups.length > 0) return;
      const config = modalityConfig[tournament.type];
      if (!config) return;
